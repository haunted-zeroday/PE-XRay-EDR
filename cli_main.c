...
void print_usage(WCHAR* app_name) {
    wprintf(L"PE-XRay Command-Line Analyzer\n");
    wprintf(L"Usage:\n");
    wprintf(L"  %s <path_to_file.exe_or_dll>   : Analyze a file.\n", app_name);
    wprintf(L"  %s --install                   : Enable context menu integration (needs admin rights).\n", app_name);
    wprintf(L"  %s --uninstall                 : Disable context menu integration (needs admin rights).\n", app_name);
...
int wmain(int argc, WCHAR** argv) {
    ...
    if (wcscmp(command, L"--help") == 0 || wcscmp(command, L"-h") == 0) {
        print_usage(argv[0]);
        return 0;
    }

    if (wcscmp(command, L"--install") == 0) {
    ...
    if (wcscmp(command, L"--uninstall") == 0) {
    ...
    WCHAR* filePathUtf8 = command;
    wprintf(L"Analyzing file: %s\n\n", filePathUtf8);
    ...
    if (analyze_pe_file(filePathUtf8, &result_data)) {
